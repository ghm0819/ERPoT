        ...
        marker_count_(0)
{
  ...
  if(!private_nh_.getParam("sensor_range", sensor_range_))
    sensor_range_ = 50.0;
  ...
  // Create a thread to periodically p4ublish the latest map->odom
...
SlamKarto::getLaser(const sensor_msgs::LaserScan::ConstPtr& scan)
{
  ...
  if(lasers_.find(scan->header.frame_id) == lasers_.end())
  {
    ...
    laser->SetRangeThreshold(sensor_range_);
...
SlamKarto::getOdomPose(karto::Pose2& karto_pose, const ros::Time& t) {
  ...
  try {
    if (std::abs(t.toSec()) <= 1e-6) {
      tf_.lookupTransform(odom_frame_, base_frame_, ros::Time(1e-6), transform);
    } else {
      tf_.lookupTransform(odom_frame_, base_frame_, t, transform);
    }
    } catch (tf::TransformException &ex) {
      ROS_ERROR("%s", ex.what());
  }
  double yaw = tf::getYaw(transform.getRotation());
  karto_pose = karto::Pose2(transform.getOrigin().x(), transform.getOrigin().y(), yaw);
  return true;
}
...
SlamKarto::updateMap()
{
  ...
  karto::OccupancyGrid* occ_grid = 
          karto::OccupancyGrid::CreateFromScans(mapper_->GetAllProcessedScans(), resolution_, occupancy_threshold_);
...
bool SlamKarto::addScan(karto::LaserRangeFinder* laser, const sensor_msgs::LaserScan::ConstPtr& scan, 
  ...
  if((processed = mapper_->Process(range_scan))) { 
    ...
    try
    {
      tf_.transformPose(odom_frame_,tf::Stamped<tf::Pose> (tf::Transform(tf::createQuaternionFromRPY(0, 0, corrected_pose.GetHeading()),
        tf::Vector3(corrected_pose.GetX(), corrected_pose.GetY(), 0.0)).inverse(), scan->header.stamp, base_frame_),odom_to_map);
    }
    catch(tf::TransformException e)
    {
